...
const Contact = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        // REAL API CALL - Connecting to your Node.js Backend
        try {
            ...
            if (response.ok) {
                // If the backend returns a 201 Created
                setStatus('success');
                setFormData({ name: '', email: '', subject: '', message: '' }); // Reset form
            } else {
                // If the backend returns a 400 Validation Error or 500 Server Error
                const errorData = await response.json();
                console.error('Backend Error:', errorData);
                setStatus('error');
            }
        } catch (error) {
            // If the frontend cannot reach the backend at all (e.g., server is offline)
